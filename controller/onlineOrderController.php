...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    $message = $_POST['message'];
   $qry = "INSERT INTO `online_order_tbl`(`name`, `address`, `email`, `contact`,`message`, `image`, `paymentRef`, `orders`, `total`, `transactionRef`, `date_Created`,`status`) 
   VALUES (?,?,?,?,?,?,?,?,?,?,NOW(), 'pending')";
   ...
   $stmt->bindParam(1, $name);
    ...
    $stmt->bindParam(7, $paymentRef);
    $stmt->bindParam(8, $orders);
    $stmt->bindParam(9, $total);
    $stmt->bindParam(10, $transactionRef);
